...
<div class="table w-full p-2">
    ...
    <table class="w-full border">
        ...
        <tbody>
            ...
            <dialog id="{{$category->id}}" class="bg-transparent z-0 relative w-screen h-screen">
                    ...
                    <div class="bg-white flex rounded-lg  relative">
                        <div class="flex flex-col justify-center">
                            <div class="p-7 flex items-center w-full">
                                <div class="text-gray-900 font-bold text-lg">{{__('Delete')}} {{$category->name}}</div>
                                <svg onclick="modalClose('{{$category->id}}')" class="ml-auto fill-current text-gray-700 w-5 h-5 cursor-pointer" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18">
                            ...
                            <div class="p-7 flex justify-end items-center w-full">
                                ...
                                <form action="{{route('categories.delete',$category->id)}}" method="POST" class="py-2 px4 mr-3">
                                    @csrf
                                    @method("DELETE")
            ...
            <tr class="bg-gray-100 text-center border-b text-sm text-gray-600">
                ...
                <td class="p-2 border-r">{{$category->img}}</td>
                <td  class="p-2 border-r">
                    <a href="{{route('categories.edit', $category->id)}}" class="bg-blue-500 p-2 text-white hover:shadow-lg text-xs font-thin">{{__('Edit')}}</a>


                </td>
                <td  class="p-2 border-r">

                        <button type="button" onclick="openModal('{{$category->id}}')" class="px-4 py-2 bg-red-500 text-white rounded mr-3">{{__('Delete')}}</button>
